const { buildTrip, mergeAI } = require('../_services/tripBuilder');
const { personalize }        = require('../_services/aiPersonalization');
...
module.exports = async (req, res) => {
  ...
  if (!trip) return res.status(404).json({ error: `No itinerary for "${destination}" (${days}d, ${tripType})` });
